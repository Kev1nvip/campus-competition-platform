...
const handleLogin = async () => {
  if (!form.username || !form.password) return (errorMsg.value = '请输入用户名和密码')
  loading.value = true
  errorMsg.value = ''
  try {
    ...
    if (res.code === 0) {
      ...
      if (userInfo.role !== 'TEACHER') return (errorMsg.value = '该账号不是教师账号')
      localStorage.setItem('token', token)
      localStorage.setItem('userInfo', JSON.stringify(userInfo))
      teacherStore.setUser({ id: String(userInfo.userId), name: userInfo.realName, tid: userInfo.username })
